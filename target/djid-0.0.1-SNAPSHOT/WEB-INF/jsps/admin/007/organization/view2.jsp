...
function goUpdate(){
	...
	if(confirm("수정하시겠습니까?")){
		$("#organFrm").attr("action", "/admin/007/organization/update2.do");
		$("#organFrm").submit();	
	}
...
function addTr(){
	...
	if(list_tr_len <= 9){
		...
		html+= '</td>';
		html+= '<td>';
		html+= '<input type="text" name="tel2" style="width: 100%;"/>';
		html+= '</td>';
		html+= '<td colspan="2">';
		...
		html+= '</td>';
		html+= '</tr>';
		$("#gubun_th").prop("rowspan", list_tr_id);
...
<div id="contents_bg">
		...
		<div class="conBox">	
			<table class="tbv" border="0" cellpadding="0" cellspacing="0" style="margin-top: 10px;">
				...
				<tbody id="list_tbody">
					<tr>
						<th colspan="6" style="padding: 10px; font-size: 16px; font-weight: bold;">${view.GUBUN}</th>
					</tr>
					<tr>
						<th>전화</th>
						<td colspan="3"><input type="text" name="tel" value="${view.TEL}" style="width: 100%;"/></td>
						<th>팩스</th>
						<td><input type="text" name="fax" value="${view.FAX}" style="width: 100%;"/></td>
					</tr>
					<tr>
						<th>구분</th>
						<th>직책</th>
						<th>성명</th>
						<th>전화</th>
						<th colspan="2">주요업무</th>
					</tr>
					
					<c:if test="${empty list}">
					<tr class="list_tr" id="list_tr1">
						<th id="gubun_th">${view.GUBUN}</th>
						<td>
						...
						<td>
							<input type="text" name="tel2" style="width: 100%;"/>
						</td>
						<td colspan="2">
					...
					<tr class="list_tr" id="list_tr${status.count}">
						<c:if test="${status.count eq 1}">
							<th id="gubun_th" rowspan="${fn:length(list)}">${view.GUBUN}</th>
						</c:if>						
						<td>
						...
						<td>
							<input type="text" name="tel2" value="${list.TEL2}" style="width: 100%;"/>
						</td>
						<c:choose>
