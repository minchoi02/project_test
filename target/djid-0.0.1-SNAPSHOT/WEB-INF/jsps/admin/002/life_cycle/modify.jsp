										...
										<td colspan="3">
											...
											<c:forEach var="list" items="${selType8List}" varStatus="status">
												<label for="type8_${status.count}">
													<input type="checkbox" name="type_code" class="type8" id="type8_${status.count}" value="${list.SEQ}" <c:if test="${fn:contains(type_8, list.CODE_DESC)}">checked</c:if>/> ${list.CODE_DESC}
												</label>
											</c:forEach>
										</td>
									</tr>									
