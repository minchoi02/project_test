									...
									<tr>
										...
										<td>
											<select id="grade" name="grade" title="장애등급 선택" style="width: 200px;">
												<option value="지체 1급" <c:if test="${view.GRADE eq '지체 1급'}">selected="selected"</c:if> >지체 1급</option>
												<option value="지체 2급" <c:if test="${view.GRADE eq '지체 2급'}">selected="selected"</c:if> >지체 2급</option>
												<option value="뇌병변 1급" <c:if test="${view.GRADE eq '뇌병변 1급'}">selected="selected"</c:if> >뇌병변 1급</option>
												<option value="뇌병변 2급" <c:if test="${view.GRADE eq '뇌병변 2급'}">selected="selected"</c:if> >뇌병변 2급</option>
											</select>											
										</td>
									</tr>
									<tr>
										...
										<td>
											<select id="person_cnt" name="person_cnt" title="탑승인원 선택" style="width: 100px;">
												<option value="1" <c:if test="${view.PERSON_CNT eq '1'}">selected="selected"</c:if> >1</option>
												<option value="2" <c:if test="${view.PERSON_CNT eq '2'}">selected="selected"</c:if> >2</option>
												<option value="3" <c:if test="${view.PERSON_CNT eq '3'}">selected="selected"</c:if> >3</option>
												<option value="4" <c:if test="${view.PERSON_CNT eq '4'}">selected="selected"</c:if> >4</option>
												<option value="5" <c:if test="${view.PERSON_CNT eq '5'}">selected="selected"</c:if> >5</option>
											</select>명							
										</td>
									</tr>
									<tr>
										...
										<td>
											<input type="radio" name="supply_yn" id="supply_y" value="Y"  <c:if test="${view.SUPPLY_YN eq 'Y'}">checked="checked"</c:if> > <label for="supply_y">수급</label>
											&nbsp;
											<input type="radio" name="supply_yn" id="supply_n" value="N" <c:if test="${view.SUPPLY_YN eq 'N'}">checked="checked"</c:if> > <label for="supply_n">미수급</label>						
										</td>
									</tr>
									<tr>
										<th>상태</th>
										<td>
											<select id="confirm_yn" name="confirm_yn" title="상태값 선택" style="width: 100px;">
												<option value="확인중" <c:if test="${view.CONFIRM_YN eq '확인중'}">selected="selected"</c:if>>확인중</option>
												<option value="예약완료" <c:if test="${view.CONFIRM_YN eq '예약완료'}">selected="selected"</c:if>>예약완료</option>
											</select>
										</td>
									</tr>
								</tbody>
