...
function Calendar(obj, argGubun, argCompareObj) {										// jucke
	...
	if (argCompareObj && argCompareObj.value != null && argCompareObj != ""){
		compareValue = argCompareObj.value;
	}
...
function validDate(sel) {
	if ( gubun != null && compareValue != null && compareValue != "" && compareValue.length==10 ){
		var selVal = sel.substring(0,4)+sel.substring(5,7)+sel.substring(8);
		var compareVal = compareValue.substring(0,4)+compareValue.substring(5,7)+compareValue.substring(8);
		
		if ( gubun == "from" ){
			if (parseInt(selVal) > parseInt(compareVal)){
...
function Show_cal(sYear, sMonth, sDay) {
	...
	Cal_HTML = "<TABLE WIDTH=100% BORDER=0 CELLPADDING=0 CELLSPACING=0 ONMOUSEOVER=doOver(); ONMOUSEOUT=doOut(); STYLE='font-size:8pt;font-family:Tahoma;'>"
			...
			+ "<TR ALIGN=CENTER BGCOLOR=#FFFFFF STYLE='color:White;font-weight:bold;'><TD class='calendar_sun'>일</TD><TD>월</TD><TD>화</TD><TD>수</TD><TD>목</TD><TD>금</TD><TD class='calendar_sat'>토</TD></TR>";
			...
			if (intThirdWeekday > 0) {											// 첫주 시작일이 1보다 크면
				...
				if (thirdPrintDay > intLastDay) {								// 입력 날짝 월말보다 크다면
					...
					if (intThisYear == NowThisYear && intThisMonth==NowThisMonth && thirdPrintDay==intThisDay) {
						Cal_HTML += "background-color:#D4DEF4;";
					}
	...
	Cal_HTML += "<tr align='center'><td colspan='7' height='25' bgcolor='#FFFFFF'><input type='button' value='닫기' onClick='Calendar(this)'></td>	</tr>";
	Cal_HTML += "</TABLE>";
