		...
		<div class="conBox">
			<table class="tbv" border="0" cellpadding="0" cellspacing="0" style="width: 930px;">
				...
				<tbody>
					...
					<c:if test="${bean.board_name == 'review' or bean.board_name == 'freeBoard' }">
						<tr>
							<td>이름</td>
							<td>${view.NAME}</td>
							<td>이메일</td>
					...
					<tr>
						<th colspan="5"><strong>질문 내용</strong></th>
					</tr>
					<tr>
					...
					<c:if test="${not empty view.ANSWER}">
					<tr>
						<th colspan="5"><strong>답변</strong></th>
					</tr>
					<tr>
						<td colspan="5" class="Tcon" style="padding: 27px; 25px;">			
							<iframe src="/admin/getAnswer.do?seq=${view.SEQ}&gubun=${view.BOARD_NAME}" style="width: 100%;" onload="setIFrameHeight(this);" scrolling="no" frameborder="0" framespacing="0" marginheight="0" marginwidth="0" vspace="0"></iframe>				
						</td>
					</tr>
					</c:if>
				</tbody>
				...
				<div class="btn_right">
					...
					<c:if test="${not empty view.ANSWER}">
						<span class="button02" id="btnSearch" ><a href="/admin/${bean.menuno}/${bean.board_name}/board_modify.do?seq=${view.SEQ}&page=${bean.page}&pagelistno=${bean.pagelistno}<c:if test="${! empty bean.search_value }">&search_name=${bean.search_name}&search_value=${bean.search_value}</c:if>" >답변수정</a></span>
					</c:if>
					<c:if test="${empty view.ANSWER}">
						<span class="button02" id="btnSearch" ><a href="/admin/${bean.menuno}/${bean.board_name}/board_modify.do?seq=${view.SEQ}&page=${bean.page}&pagelistno=${bean.pagelistno}<c:if test="${! empty bean.search_value }">&search_name=${bean.search_name}&search_value=${bean.search_value}</c:if>" >답변하기</a></span>
					</c:if>
